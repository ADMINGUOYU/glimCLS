...
This script trains the multi-task GLIM model with:
1. 2 classification tasks (sentiment, topic)
2. 2 regression tasks (length, surprisal)
3. Cosine LR scheduler with warmup
4. Support for loading from checkpoint
5. bfloat16 precision for T5 model
6. Weighted sampling by default
7. Distributed training support (DDP)
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Train GLIM_PARALLEL model (multi-task: 2 classification + 2 regression)'
    )
...
def main():
    ...
    if is_main_process():
        ...
        print("=" * 80)
        print("GLIM_PARALLEL Training (Multi-Task: 2 Classification + 2 Regression)")
        print("=" * 80)
    ...
    if args.checkpoint is not None:
        ...
        model = GLIM_PARALLEL.load_from_checkpoint(args.checkpoint)
    else:
        if is_main_process():
